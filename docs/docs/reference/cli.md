...
`canary` is one binary with 31 subcommands, listed here in registry order. `canary status` comes first on purpose: when anything else fails, it is the command that says whether the daemon is running and the gateway is reachable. Running `canary` with no arguments prints the grouped command summary.
